TMP_DIR ?=cordova-plugin-android-goto-datetime-settings
.PHONY: clean
clean:
	rm -rf /tmp/$(TMP_DIR)
...
/tmp/$(TMP_DIR)/config.xml:
	...
	cd /tmp/$(TMP_DIR) && cordova plugins add cordova-plugin-android-goto-datetime-settings --searchpath $(shell pwd)
